...
def reading_csv(train_file,test_file):
    ...
    colnames2 = ['name', 'value']
    genre = pd.read_csv('u_genre.csv', names=colnames2, header=None)
    maxvalue = genre['value'].idxmax()
    ...
    c.append(g_columns)
    c = sum(c, [])
    moviegenre = pd.read_csv('u_item.csv', names=c)
    moviegenre = moviegenre.drop('name', axis=1)
    moviegenre = moviegenre.drop('date', axis=1)
    moviegenre = moviegenre.drop('blank', axis=1)
    moviegenre = moviegenre.drop('link', axis=1)
    final_train = train.merge(moviegenre, on='movie', how='left')
    final_test = test.merge(moviegenre, on='movie', how='left')
    return train,test
...
def genMovies():
    ...
    g_c = ['name', 'value']
    genre = pd.read_csv('u_genre.csv', names=g_c, header=None)
    maxval = genre['value'].idxmax()
    columns = ['movie', 'name', 'date', 'blank', 'link']
    g_columns = range(0, maxval + 1)
